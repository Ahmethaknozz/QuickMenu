...
extension ViewController:UITableViewDelegate,UITableViewDataSource,cellyemekprotocol{
    
    func siparisver(indexpath: IndexPath) {
        let gelenyemek = yemeklerListesi[indexpath.row]
        print("Siparis Edilen Yemek: \(gelenyemek.yemekAdi!)")
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell  {
        ...
        var gelenyemek = yemeklerListesi[indexPath.row]
        var cell = tableView.dequeueReusableCell(withIdentifier: "yemeklerhucre", for: indexPath) as! cellyemekler
        cell.labelyemekad.text = gelenyemek.yemekAdi
        cell.labelyemekfiyat.text = "\(gelenyemek.yemekFiyat!) TL"
        cell.imagevieww.image = UIImage(named: gelenyemek.yemekResimAdi!)
